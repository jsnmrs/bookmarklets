...
function traverseFrames(doc, _framesObj) {
  // check for autocomplete attribute in current document and then check it's frames

  displayAutoComplete(doc, _framesObj);
...
function displayAutoComplete(doc, _framesObj) {
  var el = doc.querySelectorAll("[autocomplete]");
  ...
  if (el.length > 0) {
    for (var i = 0; i < el.length; i++) {
      ...
      t = doc.createTextNode(" Autocomplete=" + el.item(i).autocomplete + " ");
      s.appendChild(t);
    ...
    //  alert('no elements with auto complete');
  }
}
...
function run(doc) {
  ...
  alert(
    _framesObj.foundCount + " element(s) with autocomplete attribute found. "
  );
}
