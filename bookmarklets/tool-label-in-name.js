...
function findLableForInput(doc, el) {
  ...
  for (var i = 0; i < labels.length; i++) {
    if (labels[i].htmlFor == idValue) return labels[i];
  }
}
// ***********************************************
function showLabels(doc) {
  //var col = doc.getElementsByTagName('img');
  var col = doc.querySelectorAll(
    "button, input, textarea, select, [tabindex='0'],a[href],summary"
  );
  var props;
  var text;
  ...
  for (var i = 0; i < col.length; i++) {
    ...
    if (
      (col[i].tagName == "INPUT" ||
        col[i].tagName == "SELECT" ||
        col[i].tagName == "TEXTAREA") &&
      findLableForInput(doc, col[i])
    ) {
